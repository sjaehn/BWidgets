...
ShapeWidget::ShapeWidget (const double x, const double y, const double width, const double height, const std::string& name) :
		...
		PointerFocusable (std::chrono::milliseconds (2000),
...
ShapeWidget::ShapeWidget (const ShapeWidget& that) :
		Shape (that), ValueWidget (that), PointerFocusable (that),
...
void ShapeWidget::onButtonPressed (BEvents::PointerEvent* event)
{
	...
	if (event->getButton() == BDevices::Left)
	{
...
void ShapeWidget::onValueChanged (BEvents::ValueChangedEvent* event)
{
	...
	{
		update ();
		cbfunction_[BEvents::EventType::valueChangedEvent] (event);
		setValue (0);
	}
}

void ShapeWidget::onFocusIn (BEvents::PointerFocusEvent* event)
{
...
void ShapeWidget::onFocusOut (BEvents::PointerFocusEvent* event)
{
