                         ...
                         const BStyles::Color fgColor, const BStyles::Color bgColor)
{
    ...
    // Frame
    cairo_set_line_width (cr, 0.9 * height);
    cairo_set_line_cap (cr, CAIRO_LINE_CAP_ROUND);

    cairo_move_to (cr, x0 + 0.45 * height, y0 + 0.45 * height);
    cairo_line_to (cr, x0 + 0.45 * height < x0 + width - 0.55 * height ? x0 + width - 0.55 * height : x0 + 0.45 * height, y0 + 0.45 * height);
    cairo_set_source_rgba (cr, CAIRO_RGBA(bgColor.illuminate(BStyles::Color::darkened)));
    cairo_stroke (cr);

    cairo_move_to (cr, x0 + 0.55 * height, y0 + 0.55 * height);
    cairo_line_to (cr, x0 + 0.55 * height < x0 + width - 0.45 * height ? x0 + width - 0.45 * height : x0 + 0.55 * height, y0 + 0.55 * height);
